...
exports.save = async (req, res) => {
    const { product_type, category, Product_name, Description, patti, Quantity_patti, Single_pcs, price, QtyInCartoon, totalAmount, productid, subjectids } = req.body;
                                ...
                                if (QtyInCartoon && QtyInCartoon != '' && QtyInCartoon != null && QtyInCartoon != undefined) {
                                    if (totalAmount != null && totalAmount != undefined) {
                                        if (productid && productid.trim() != '' && productid != null && productid != undefined && mongoose.Types.ObjectId.isValid(productid)) {
                                            let checkExist = await primary.model(constants.Model.products, productmodel).findOne({ _id: { $ne: new mongoose.Types.ObjectId(productid) }, Product_name: Product_name }).lean();
                                            if (checkExist == null) {
                                            ...
                                            let checkExist = await primary.model(constants.Model.products, productmodel).findOne({ Product_name: new RegExp(["^", Product_name, "$"].join(""), "i") }).lean();
                                            if (checkExist == null) {
                                                let amount = price * QtyInCartoon;
                                                let obj = {
                                                    product_type: product_type,
